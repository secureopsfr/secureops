"""Worker de consommation des jobs async crawl."""
...
from app.services.async_crawl_executor import execute_crawl_job
from app.services.async_job_repository import append_job_progress_batch, claim_next_job, mark_completed, mark_failed
...
async def _mark_timeout(session: Any, job: Any) -> None:
    await mark_failed(
        session,
        job,
...
async def _mark_failure_from_error(session: Any, job: Any, error: dict[str, Any]) -> None:
    await mark_failed(
        session,
        job,
        message=str(error.get("message", "Erreur crawl")),
        status_code=int(error.get("status_code", 500)),
        error_type=str(error.get("error_type", "crawl_error")),
        retryable=True,
    )
...
async def _finalize_job(
    ...
    *,
    ...
    if utc_now() - started_at > timedelta(seconds=JOB_TIMEOUT_SECONDS):
        await _mark_timeout(session, job)
        return
    if error:
...
async def _execute_claimed_job(session: Any, job: Any) -> None:
    ...
    try:
        ...
        logger.exception("crawl-worker: unexpected failure job=%s", job.id)
        await progress.flush(force=True)
        await _mark_unexpected_failure(session, job, exc)


async def _run_once() -> bool:
    """Process at most one crawl async job from the queue."""
...
async def main() -> None:
    """Run the crawl worker loop indefinitely."""
    logging.basicConfig(level=logging.INFO)
    db_ok = await init_db()
    if not db_ok:
        raise RuntimeError("DATABASE_URL manquante pour crawl-worker")
    while True:

...
from app.services.crawler.core import CrawlUrlEntry, run_crawl
...
def _empty_crawl_result() -> tuple[list[CrawlUrlEntry], bool, bool, bool, list[str]]:
...
async def _run_crawler_impl(
    ...
    entries, timeout_reached, requests_blocked, disallow_paths = await run_crawl(url, max_urls=max_urls, on_progress=cb, stop_event=stop_ev)

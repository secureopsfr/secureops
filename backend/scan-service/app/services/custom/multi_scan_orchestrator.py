"""Custom multi-URL orchestration based on reusable pipeline base."""
...
class CustomMultiScanOrchestrator(BaseMultiScanOrchestrator):
    """Custom implementation using the reusable multi-scan template."""
    ...
    async def after_client_run(
        ...
        *,
        ...
        """Publish custom multi-scan HTTP metrics once execution completes."""
        log_http_metrics(client, "custom-multi-scan", base_url=base_url, urls=len(urls))
    ...
    async def run_domain_phase(
        ...
        """Run domain-level custom checks and return reusable intermediate results."""
        _ = client
    ...
    async def run_single_page(
        ...
        *,
        ...
        """Run custom checks for one page and return its page-level result object."""
        ...
        try:
            ...
            logger.warning("custom_multi_scan: page inaccessible url=%s err=%s", url, exc)
            await self.emit_progress("page_scan_error", url=url)
            return PageScanResult(url=url, score=0, findings=[], error="Page inaccessible for custom checks")
    ...
    def build_final_result(
        ...
        *,
        ...
        """Build and return the final aggregate custom multi-scan result."""
...
async def run_multi_scan(
    ...
    """Execute a custom multi-URL scan with shared orchestration logic."""
    orchestrator = CustomMultiScanOrchestrator(on_progress=on_progress)
    return await orchestrator.run(urls)


async def validate_multi_scan_urls(urls: list[str]) -> list[str]:
    """Validate custom multi-scan targets with shared preflight rules."""

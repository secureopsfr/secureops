...
async def test_run_tls_checks_https_ok_et_redirect_ok() -> None:
    ...
    with (
        patch("app.services.passive.both.tls.checks.httpx.AsyncClient") as mock_client,
        patch("app.services.passive.both.tls.checks.fetch_certificate_chain", return_value=[]),
        patch("app.services.passive.both.tls.checks.fetch_certificate_der", return_value=valid_cert),
        patch("app.services.passive.both.tls.checks.check_tls_versions_obsolete", return_value=([], [])),
    ):
...
async def test_run_tls_checks_https_non_actif_connect_error() -> None:
    ...
    with patch("app.services.passive.both.tls.checks.httpx.AsyncClient") as mock_client:
        mock_context = AsyncMock()
        mock_context.__aenter__ = AsyncMock(return_value=mock_client_instance)
        mock_context.__aexit__ = AsyncMock(return_value=None)
        mock_client.return_value = mock_context
...
async def test_run_tls_checks_certificat_expire() -> None:
    ...
    with (
        patch("app.services.passive.both.tls.checks.httpx.AsyncClient") as mock_client,
        patch("app.services.passive.both.tls.checks.fetch_certificate_chain", return_value=[]),
        patch("app.services.passive.both.tls.checks.fetch_certificate_der", return_value=expired_cert),
        patch("app.services.passive.both.tls.checks.check_tls_versions_obsolete", return_value=([], [])),
    ):
        mock_context = AsyncMock()
        mock_context.__aenter__ = AsyncMock(return_value=mock_client_instance)
...
async def test_run_tls_checks_certificat_auto_signe() -> None:
    ...
    with (
        patch("app.services.passive.both.tls.checks.httpx.AsyncClient") as mock_client,
        patch("app.services.passive.both.tls.checks.fetch_certificate_chain", return_value=[]),
        patch("app.services.passive.both.tls.checks.fetch_certificate_der", return_value=self_signed_cert),
        patch("app.services.passive.both.tls.checks.check_tls_versions_obsolete", return_value=([], [])),
    ):
        mock_context = AsyncMock()
        mock_context.__aenter__ = AsyncMock(return_value=mock_client_instance)
...
async def test_run_tls_checks_tls_versions_obsoletes() -> None:
    ...
    with (
        ...
        patch("app.services.passive.both.tls.checks.fetch_certificate_der", return_value=valid_cert),
        patch(
            "app.services.passive.both.tls.checks.check_tls_versions_obsolete",

"""Tests unitaires pour les vérifications sitemap (app.services.sitemap.checks)."""
...
from app.services.sitemap import SensitiveSitemapUrl, SitemapCheckResult, run_sitemap_checks


@pytest.mark.asyncio()
async def test_run_sitemap_checks_no_sitemap_found() -> None:
    """Aucun sitemap trouvé (ni déclaré ni fallback) → sitemap_found False."""
    with patch("app.services.sitemap.checks.get_with_client", new_callable=AsyncMock) as mock_get:
